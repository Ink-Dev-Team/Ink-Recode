package com.ink.recode.gui.clickgui.panel;

import com.ink.recode.Category;
import com.ink.recode.Module;
import com.ink.recode.ModuleManager;
import com.ink.recode.gui.IComponent;
import com.ink.recode.gui.clickgui.component.ModuleComponent;
...
public class CategoryPanel implements IComponent {
    ...
    public CategoryPanel(Category category) {
        ...
        for (i = 0; i < (ModuleManager.getModulesByCategory(category).size()); ++i) {
            Module module = ModuleManager.getModulesByCategory(category).get(i);
    ...
    public void render(DrawContext guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        NanoVGRenderer.INSTANCE.draw(vg -> {
            ...
            NanoVGHelper.drawString(category.getDisplayName(), x + 4 * guiScale, y + 12f * guiScale, FontLoader.bold(baseFontSize), baseFontSize, new Color(255, 255, 255, 255));
            float iconSize = baseFontSize * 1.5f;
            NanoVGHelper.drawString(category.getIcon(), x + 4 * guiScale + scaledWidth - NanoVGHelper.getTextWidth(category.getIcon(), FontLoader.icons(iconSize), iconSize) - (category == Category.RENDER ? 7 : 3) * guiScale, y + 13f * guiScale, FontLoader.icons(iconSize), iconSize, new Color(255, 255, 255, 255));
        });
