...
// 1.16及更早用这个：import net.minecraft.util.EnumFacing
// 1.17+用这个：import net.minecraft.util.Direction
...
object RotationUtils {
    ...
    // 🔧 统一安全获取眼高，兼容所有版本
    private fun getEyeHeight(entity: LivingEntity): Double {
        // 1.16及更早：return entity.getEyeHeight()
        // 1.17+：return entity.getEyeHeight(entity.pose) 或 entity.eyeHeight
        return entity.getEyeHeight(entity.pose).toDouble()
    }
    ...
    fun getRotation(entity: Entity): Rotation {
        ...
        // ✅ 用安全方法获取眼高，替代直接属性
        val playerEyeHeight = getEyeHeight(player)
        val entityEyeHeight = (entity as? LivingEntity)?.let { getEyeHeight(it) } ?: 1.0
        ...
        val pitch = MathHelper.wrapDegrees(-Math.toDegrees(Math.atan2(diff.y, sqrt(diff.x * diff.x + diff.z * diff.z))))
    ...
    // ✅ 删除冗余的Direction参数，彻底解决Direction报错
    fun calculate(pos: BlockPos): Rotation {
        val player = mc.player ?: return Rotation.ZERO
        val eyesPos = Vec3d(player.x, player.y + getEyeHeight(player), player.z)
        ...
        val pitch = -Math.toDegrees(Math.atan2(lookVec.y, sqrt(lookVec.x * lookVec.x + lookVec.z * lookVec.z)))
